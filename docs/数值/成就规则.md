...
>
> 时间口径：当前回合为“月”，`weeks_survived` 字段名保留，但语义按“累计月数”解释。
...
- `player_weekly_profit_total`：**店铺利润汇总**（用于比较/数值，不含生活费等玩家侧支出）
...
- `player_weekly_profit_total` 在成就语境里统一解释为 **店铺利润汇总**，不是单店利润（单店用 `shop_weekly_profit`）。
