...
- 年时间口径：统一用 `MONTHS_PER_YEAR = 12`（游戏口径）
- 注：历史字段名里仍保留 `week/weekly` 等命名，但 **语义按“月口径”解释**（避免大范围改表）。

### 0.2 存档值 vs 派生值

- **存档值（state）**：玩家/店铺/员工/世界的“状态”，回合推进需要连续性（例如 `cash`、`stress`、员工列表、flags）。
- **派生值（derived）**：由存档值计算出来的展示/排行/结算结果，不建议落库（例如 `player_monthly_profit_total`、`total_asset`）。
...
2. **生成事件候选**：根据状态从 `docs/数值/events*.csv` 按 `pools_json/conditions_json/occurrence_json/weight_rules_json` 生成候选并抽取（每次交互抽取 1 个）
3. **玩家选择与检定**：根据 `docs/数值/choices*.csv` 先用 `visibility_json` 决定可见选项，再按 `resolution_json` 判定成功/失败，并应用 `outcomes_json.effects`（含 `stat/flag/权重修正` 等）
...
7. **时间推进**：`current_month += 1`；每 12 月 `age += 1`；世界事件持续时间递减
8. **死亡/破产判定**：破产、压力、疾病、年龄等（见第 13 章判定）

交互节奏约定（与你当前决定对齐）：

- **每次交互固定 1 个事件**（世界事件为“额外系统”，不占用这 1 个名额）
- 事件/世界事件可触发“自动推进多月”，并按月逐月执行第 4~8 步；自动推进期间不再抽取新的交互事件
...
| 数值名 | 类型 | 初始值 | 范围 | 说明 |
|--------|------|--------|------|------|
| `cash` | 整数 | `INITIAL_CASH`（默认 100,000） | 0 ~ ∞ | 游戏币/现金，归0则破产；具体数值可后续平衡调整，但文档与实现统一从配置常量读取 |
| `debt` | 整数 | 0 | 0 ~ ∞ | 负债口径（**派生值**）：`debt = Σ loans[].remaining_principal`（可落库但必须保持同步） |

#### 1.1.1 贷款（等额本息，唯一负债口径）

> 你已明确：贷款为**固定期限的等额本息**。因此不再支持“一个总 debt + 每月固定还款额”的旧口径，统一用 `loans[]` 作为唯一真实负债来源（存档值）。

建议存档值：

| 数值名 | 类型 | 说明 |
|--------|------|------|
| `loans[]` | 数组 | 贷款列表（每笔独立期限/利率/剩余本金） |
...
|------|------|------|
| `principal` | 整数 | 本金（元） |
| `remaining_principal` | 整数 | 剩余本金（元） |
| `annual_rate` | 浮点 | 年化利率（0~1） |
| `term_months` | 整数 | 期限（月） |
| `monthly_payment` | 整数 | 每月应还（元，等额本息，取整） |

每月扣款（月结算阶段）建议：

- 月利率：`monthly_rate = annual_rate / MONTHS_PER_YEAR`（游戏口径：1年 = 12月）
- 本月利息：`interest = floor(remaining_principal * monthly_rate)`
- 本月应扣：`payment = min(monthly_payment, remaining_principal + interest)`
- 本月本金：`principal_paid = max(0, payment - interest)`
- 更新：`remaining_principal -= principal_paid`

`debt` 字段仅作为“所有贷款剩余本金之和”的缓存/展示（方便 UI 与压力计算），不要作为结算输入的单一真相来源。

### 1.2 状态类

| 数值名 | 类型 | 初始值 | 范围 | 说明 |
|--------|------|--------|------|------|
| `stress` | 整数 | `INITIAL_STRESS`（默认 0） | 0 ~ 100 | 压力值；当 `stress = 100` **连续满值**达到阈值会死亡（默认阈值见 `STRESS_DEATH_MONTHS = 4`） |
| `stress_max_months` | 整数 | 0 | 0 ~ ∞ | 压力满值持续月数（**派生/运行时计数**）；达到 `STRESS_DEATH_MONTHS` 触发死亡，建议不作为“单一真相来源”落库 |
| `health` | 整数 | 100 | 0 ~ 100 | 健康值，影响疾病概率和工作效率 |
| `age` | 整数 | 25 | 18 ~ 100 | 年龄，每12月+1岁（游戏口径：1年 = 12月），到达上限去世 |
| `energy` | 整数 | 100 | 0 ~ 100 | 精力值，影响每月可做的事情数量 |
...
| `living_expense` | 整数 | 2000 | 1000 ~ ∞ | 每月生活费，随生活状态变化 |

### 1.4 玩家派生值（不建议存档）

| 数值名 | 类型 | 说明 |
|--------|------|------|
| `player_monthly_profit_total` | 整数 | 所有店铺本月利润汇总（不含玩家生活/家庭/债务） |
| `monthly_net_cashflow` | 整数 | 玩家本月净现金流（店铺利润汇总 - 生活/家庭/债务 - 事件现金变化） |
...
> 为避免数值“叠加过猛/难平衡”，本项目**只启用 16 型表（2.3）作为最终生效口径**。
>
> 本节的“四维度效果”仅作为**策划推导/拆解来源**（用于解释为什么某个 16 型会有对应效果），实现时**不再额外叠加生效**。

| 维度 | 效果 |
|------|------|
| **E（外向）** | 社交事件成功率+10%，招聘耗时 -1 周，但独处时压力恢复-20% |
...
| `shop_weekly_profit` | 整数 | -∞ ~ ∞ | 单店周净利润（派生，见第 13 章） |
| `shop_rating` | 浮点 | 1.0 ~ 5.0 | 店铺评分（影响客流） |
| `hygiene_score` | 整数 | 0 ~ 100 | 卫生评分（影响检查事件） |

#### 3.3.1 人流 → 客流映射（补齐口径，避免“选址很重要但落不到结果”）

> 说明：门面系统里既有“位置类型/点位倍率”，也有隐藏的 `foot_traffic/competition_index` 等细分属性。为了让经营可控、易平衡，经营侧建议**只使用一条稳定的映射公式**把这些输入统一折算到 `daily_customers`。

推荐实现（线性 + 少量指数，便于策划调参）：

```
// 基础日客流（由店铺类型给出，例如 milk_tea/noodle 的 base_customers_by_type）
base_daily_customers = base_customers_by_type

// 选址倍率：位置类型 × 点位优劣（来自 4.2 的 traffic_multiplier 与 core/poor_traffic_mult）
traffic_mult = location_traffic_mult × spot_traffic_mult

// 竞争折损：competition_index 越高，客流越被分走
competition_mult = clamp(1 - (competition_index / 100) × COMPETITION_B, 0.6, 1.0)

// 评分加成：评分越高越能“吃满人流”（指数项避免 4.9~5.0 过强线性爆炸）
rating_mult = (shop_rating / 5) ^ RATING_EXP

daily_customers =
  floor(base_daily_customers × traffic_mult × rating_mult × competition_mult)
```
...
| `delivery_commission` | delivery_revenue × commission_rate | 平台抽成（周口径，**派生中间量**，不落库；用于解释口径，最终成本汇总不要重复计算） |
...
| 数值名 | 类型 | 说明 |
|--------|------|------|
| `current_week` | 整数 | 当前月数（字段名保留，语义为“月”；从1开始） |
| `current_year` | 整数 | 当前年份 |
| `season` | 枚举 | spring/summer/autumn/winter |
| `is_holiday` | 布尔 | 是否节假日 |
| `holiday_type` | 枚举 | 节日类型（影响客流） |

### 8.2 事件触发与抽取（对齐 `docs/数值/events.csv` 新表头 v1）

本项目建议以 `docs/数值/events.csv` 为**事件触发的唯一数据源**。新表头把“筛选/次数控制/权重规则”结构化到多个 `_json` 列中（字段解释见 `docs/数值/字段字典.md`）。

事件抽取流程建议：

1. **确定事件池（bag）与阶段（stage）**：由当前游戏进度决定（例如 `early_daily`、`customer_conflict`）
2. **按池过滤候选**：筛选 `pools_json` 中包含 `bag_id` 且 `stage_min<=stage<=stage_max` 的事件
3. **按条件过滤候选**：用 `conditions_json` 做范围/列表/flags 过滤（例如 `week/cash/reputation/stress/shop_type_in/required_flags_*`，其中 `week` 语义为“月”）
4. **按次数控制过滤候选**：用 `occurrence_json` 应用冷却与次数限制（例如 `cooldown_turns/max_total/max_per_stage/once_only`）
5. **计算最终权重**：
   - 基础权重：来自 `pools_json` 的对应条目 `weight`
   - 状态修正：遍历 `weight_rules_json`，命中 `when` 条件后按 `op/value` 修正（例如 `mul × 1.3`）
   - 运行时修正：若玩家在历史选项里获得过临时修正（例如 `bag_weight/event_weight`），在这里叠加
6. **加权随机抽取事件**：每次交互抽取 1 个
7. **拉取选项**：用 `docs/数值/choices.csv` 按 `event_id` 拉取该事件的所有选项，进入展示与交互

阶段（stage）口径补充（用于解决“目标 10-无限回合”歧义，并与第 14.1 经济阶段对齐）：

- `stage` 由 `current_week` 派生得到（不落库；语义为“月”）：  
  - `stage = 1`：第 1~12 月（新手期）  
  - `stage = 2`：第 13~36 月（成长期）  
  - `stage = 3`：第 37~120 月（稳定期）  
  - `stage = 4`：第 121 月及以后（扩张期）

### 8.3 选项结果与数值效果（对齐 `docs/数值/choices.csv` 新表头 v1）

新表头将“数值变化/flags/权重修正/事件链”等统一表达为 `outcomes_json.effects`。为避免“事件阶段已改数值 + 结算阶段再叠加一次”导致双算，建议实现时采用以下口径：

- **事件阶段（第3步）直接应用 `effects` 到核心数值**（例如 `cash/stress/health`）
- 如需 UI 展示“本次事件带来的变化”，可在事件阶段**临时汇总** `event_delta_*` 作为展示用派生值，但**不得在第 5/6 步再次用于结算叠加**

`effects` 常用作用域建议：

| `scope` | `target` 示例 | `op` | 说明 |
|--------|---------------|------|------|
| `stat` | `"cash"` | `add/mul` | 玩家核心数值变化（金额用整数；倍率用浮点） |
| `flag` | `"customer_angry"` | `add` | flags 计数变化；可带 `duration_turns` 表示到期自动回滚/移除 |
| `bag_weight` | `{ "bag_id":"customer_conflict" }` | `mul` | 临时修正某个 bag 的抽取权重（持续若干月） |
| `event_weight` | `{ "tags_any":["投诉"] }` | `mul` | 临时修正“满足标签条件”的事件权重（持续若干月） |

#### 8.3.1 选项可见性与检定字段（新表头 v1）

- `visibility_json`：决定选项是否显示（条件口径与 `events.csv:conditions_json` 一致）
- `resolution_json`：决定成功/失败如何判定
  - `type="prob"`：按 `success_rate (0~1)` 做概率检定
  - 后续如需 MBTI/技能加成，建议把修正项写进 `resolution_json` 或在外层统一解释器里处理（避免散落在多个 CSV 列）
- `outcomes_json`：至少提供 `success` 分支；若提供 `fail` 分支则在失败时生效
  - 如需要“后续事件/事件链”，建议在 `success/fail` 内新增 `next_event_id`（或用一个 `scope` 专门表达事件队列），并在解释器里统一处理
...
> 说明（命名强约束）：文档与数据表里，“周利润”**必须写全**，禁止使用简称字段名。
>
> - 单店周利润：`shop_weekly_profit`
> - 玩家侧店铺利润汇总（不含生活费/贷款/事件现金等）：`player_weekly_profit_total`
...
| 成就ID | 名称 | 条件（示例） | 奖励（示例） |
|--------|------|--------------|--------------|
| `A001` | 第一桶金 | `player_weekly_profit_total > 0` | `unlock: new_game_mode` |
...
| 周榜 | `player_weekly_profit_total` | 本周店铺利润汇总排名（不含生活费等玩家侧支出） |
...
```
MONTHS_PER_YEAR = 12
DAYS_PER_MONTH = 30  // 游戏口径：1年 = 12个月

// 单店堂食：日营业额
dine_in_daily_revenue = daily_customers × avg_ticket

// 单店堂食：月营业额（回合口径）
dine_in_monthly_revenue = dine_in_daily_revenue
                      × DAYS_PER_MONTH
                      × season_modifier
                      × holiday_modifier

// 单店外卖：月营业额（回合口径）
delivery_revenue = weekly_orders × delivery_avg_ticket  // 字段名保留，语义为“月订单量”

// 单店总月营业额
shop_monthly_revenue = dine_in_monthly_revenue + delivery_revenue

// 全部店铺总月营业额
total_monthly_revenue = Σ(each_shop.shop_monthly_revenue)
```
...
```
// 单店月固定成本（按月口径）
shop_monthly_fixed_cost =
  (monthly_rent + monthly_labor_cost + monthly_utility_cost
   + monthly_marketing_cost + monthly_brand_fee + monthly_other_cost)

// 单店月原料/损耗等变动成本（由毛利率统一表达）
shop_monthly_cogs = shop_monthly_revenue × (1 - gross_margin)

// 单店加盟抽成（仅加盟店；按营业额抽成）
brand_royalty = (operation_mode == "franchise" ? shop_monthly_revenue × royalty_rate : 0)

// 单店月外卖成本（抽成 + 推广预算 + 平台固定费；其余后续再细化）
// 注意：delivery_commission 是派生中间量；下式采用“展开式”写法，最终汇总时不要把 delivery_commission 再单独加一次。
delivery_commission = delivery_revenue × commission_rate
shop_monthly_delivery_cost =
  delivery_commission
  + monthly_promo_budget
  + platform_fixed_monthly_fee

// 单店总月成本
shop_monthly_cost = shop_monthly_fixed_cost + shop_monthly_cogs + brand_royalty + shop_monthly_delivery_cost

// 玩家侧总月成本（生活/家庭等）
player_monthly_personal_cost = living_expense + family_expense

// 贷款月成本（等额本息，多笔）
loan_interest_i = floor(loan_i.remaining_principal × loan_i.annual_rate / MONTHS_PER_YEAR)
loan_payment_i = min(loan_i.monthly_payment, loan_i.remaining_principal + loan_interest_i)
loan_principal_i = max(0, loan_payment_i - loan_interest_i)

loan_monthly_interest_total = Σ(loan_interest_i)
loan_monthly_payment_total = Σ(loan_payment_i)
loan_monthly_principal_total = Σ(loan_principal_i)
```
...
```
// 单店月利润
shop_monthly_profit = shop_monthly_revenue - shop_monthly_cost

// 店铺利润汇总（派生值）
player_monthly_profit_total = Σ(each_shop.shop_monthly_profit)

// 玩家本月净现金流（派生值）
// 注意：事件阶段（第3步）对 cash 的变化应当“即时生效”，不应再在这里通过 event_delta_cash 叠加一次，否则会双算。
monthly_net_cashflow =
  player_monthly_profit_total
  - player_monthly_personal_cost
  - loan_monthly_payment_total

// 玩家现金更新（存档值）
cash = cash + monthly_net_cashflow

// 玩家债务口径（派生值；如实现选择落库，也必须按该口径同步）
debt = Σ(each_loan.remaining_principal)
```
...
// 压力死亡
if (stress >= 100) stress_max_months++
else stress_max_months = 0
if (stress_max_months >= STRESS_DEATH_MONTHS) trigger_death("stress")
...
- [ ] 初始启动资金具体数值（当前默认见 `INITIAL_CASH=100000`，后续平衡调整以改配置为准）
- [ ] 每种店铺类型的详细数值表
- [ ] 品牌数据库（加盟品牌列表）
- [ ] 事件池完整内容
- [ ] 成就完整列表
