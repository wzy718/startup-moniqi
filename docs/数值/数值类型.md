...
- 年时间口径：统一用 `WEEKS_PER_YEAR = 48`（游戏口径，12个月×4周，与成本口径对齐）

### 0.2 存档值 vs 派生值

- **存档值（state）**：玩家/店铺/员工/世界的“状态”，回合推进需要连续性（例如 `cash`、`stress`、员工列表、flags）。
- **派生值（derived）**：由存档值计算出来的展示/排行/结算结果，不建议落库（例如 `player_weekly_profit_total`、`total_asset`）。
...
> 你已明确：贷款为**固定期限的等额本息**。因此不再支持“一个总 debt + 每周固定还款额”的旧口径，统一用 `loans[]` 作为唯一真实负债来源（存档值）。
...
| `term_weeks` | 整数 | 期限（周） |
| `weekly_payment` | 整数 | 每周应还（元，等额本息，取整） |

每周扣款（周结算阶段）建议：

- 周利率：`weekly_rate = annual_rate / WEEKS_PER_YEAR`（游戏口径：1年 = 48周）
- 本周利息：`interest = floor(remaining_principal * weekly_rate)`
- 本周应扣：`payment = min(weekly_payment, remaining_principal + interest)`
- 本周本金：`principal_paid = max(0, payment - interest)`
...
| 数值名 | 类型 | 初始值 | 范围 | 说明 |
|--------|------|--------|------|------|
| `stress` | 整数 | `INITIAL_STRESS`（默认 0） | 0 ~ 100 | 压力值；当 `stress = 100` **连续满值**达到阈值会死亡（默认阈值见 `STRESS_DEATH_WEEKS = 4`） |
| `stress_max_weeks` | 整数 | 0 | 0 ~ ∞ | 压力满值持续周数（**派生/运行时计数**）；达到 `STRESS_DEATH_WEEKS` 触发死亡，建议不作为“单一真相来源”落库 |
| `health` | 整数 | 100 | 0 ~ 100 | 健康值，影响疾病概率和工作效率 |
| `age` | 整数 | 25 | 18 ~ 100 | 年龄，每48周+1岁（游戏口径：1年 = 12个月 = 48周），到达上限去世 |
...
| 数值名 | 类型 | 说明 |
|--------|------|------|
| `player_weekly_profit_total` | 整数 | 所有店铺本周利润汇总（不含玩家生活/家庭/债务） |
...
3. **按条件过滤候选**：用 `conditions_json` 做范围/列表/flags 过滤（例如 `week/cash/reputation/stress/shop_type_in/required_flags_*`）
...
- `stage` 由 `current_week` 派生得到（不落库）：  
  - `stage = 1`：第 1~12 周（新手期）  
  - `stage = 2`：第 13~52 周（成长期）  
  - `stage = 3`：第 53~156 周（稳定期）  
  - `stage = 4`：第 157 周及以后（扩张期）
...
| `bag_weight` | `{ "bag_id":"customer_conflict" }` | `mul` | 临时修正某个 bag 的抽取权重（持续若干周） |
| `event_weight` | `{ "tags_any":["投诉"] }` | `mul` | 临时修正“满足标签条件”的事件权重（持续若干周） |
...
```
WEEKS_PER_MONTH = 4
WEEKS_PER_YEAR = 48  // 游戏口径：1年 = 12个月 = 48周
...
// 单店总周成本
shop_weekly_cost = shop_weekly_fixed_cost + shop_weekly_cogs + brand_royalty + shop_weekly_delivery_cost

// 玩家侧总周成本（生活/家庭等）
player_weekly_personal_cost = living_expense + family_expense

// 贷款周成本（等额本息，多笔）
loan_interest_i = floor(loan_i.remaining_principal × loan_i.annual_rate / WEEKS_PER_YEAR)
loan_payment_i = min(loan_i.weekly_payment, loan_i.remaining_principal + loan_interest_i)
loan_principal_i = max(0, loan_payment_i - loan_interest_i)

loan_weekly_interest_total = Σ(loan_interest_i)
loan_weekly_payment_total = Σ(loan_payment_i)
loan_weekly_principal_total = Σ(loan_principal_i)
```
...
// 店铺利润汇总（派生值）
player_weekly_profit_total = Σ(each_shop.shop_weekly_profit)

// 玩家本周净现金流（派生值）
// 注意：事件阶段（第3步）对 cash 的变化应当“即时生效”，不应再在这里通过 event_delta_cash 叠加一次，否则会双算。
weekly_net_cashflow =
  player_weekly_profit_total
  - player_weekly_personal_cost
  - loan_weekly_payment_total
...
if (stress_max_weeks >= STRESS_DEATH_WEEKS) trigger_death("stress")
