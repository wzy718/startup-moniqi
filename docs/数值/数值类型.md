...
#### 1.1.2 贷款（等额本息，推荐实现口径）

> 你已明确：贷款为**固定期限的等额本息**。因此建议不要用“一个总 debt + 每周固定还款额”去近似，而是存档里按“多笔贷款”管理。
...
`debt` 字段可继续保留为“所有贷款剩余本金之和”（方便展示与压力计算）。
...
// 若采用“贷款列表 loans[]（等额本息）”口径：
// - loan_weekly_payment_total = Σ(each_loan.payment)
