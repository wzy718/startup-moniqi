...
- 触发方式：每周结算开始时，对“当前未激活”的世界事件逐个做一次概率检定 `random() < probability`。
- `duration_min/duration_max`：
  - `>= 1`：随机一个整数周数作为持续时间
  - `-1`：永久（直到游戏结束或你设计解除事件）
- `trigger_condition`：只支持一个**安全子集**（避免写成任意脚本）：
  - `none`
  - `week>NUM`、`week>=NUM`、`week<NUM`、`week<=NUM`
  - `cash>NUM`（玩家现金条件）
...
- `weekly_orders *= delivery_traffic_mult`
- `avg_ticket *= avg_ticket_mult`（或只作用在结算的派生值上，不回写存档）

### 3.2 成本

- `labor_cost_mult`：人工成本倍率（默认 `1.0`），作用在 `shop.monthly_labor_cost`
- `cogs_cost_mult`：原料/损耗倍率（默认 `1.0`），作用在 `shop_weekly_cogs`
- `weekly_fixed_cost_add`：每周固定额外成本（元）
- `one_time_cost_add`：一次性成本（元），在事件触发当周扣除（或均摊到持续期也可，但要写死规则）

### 3.3 特殊/标记

- `forced_close_chance`：强制关店概率（如触发，则当周堂食/外卖归零，或直接跳过经营结算）
